...
@Component({
  selector: 'app-calling-list',
  templateUrl: './calling-list.component.html',
  styleUrls: ['./calling-list.component.css']
})
export class CallingListComponent extends BaseTabComponent {
  ...
  public distancetOptions = [
   'Any Distance', '5','10', '12','15','17','20', '25','30', '35', '40', '45', '50', '75', '100', '200', 'state', 'country'
  ]
  ...
  employmentOption: String = "Any Empoyment Type"
  ...
  // }
  initializeFields(): void {
    this.setParentAttributes({ "api_end_point": this.api_end_point, "sp": "", "table_name": "jobapplication", "display_on_load": false })
  ...
  constructor(@Inject(MAT_DIALOG_DATA) public data: any, paramService: ParamService, protected dataService: GenericDataService, protected popupService: PopupService, private dataSharingService: DataSharingService, public dialog: MatDialog) {
    ...
    this.dataService.fetchDataPost('get_elastic_search_titles', null, {}).subscribe(data => {
      this.normalizedTitleOptions = data
    });
...
get sendLabel(): string {
  ...
  validateOptions(): boolean {
    // Check if distancetOption is null
    if (this.distancetOption === null) {
        alert('Distance option must not be null.');
        return false;
    }

    // Check if distancetOption contains 'Any'must
    if (this.distancetOption.includes('Any')) {
        alert('Distance must be selected.');
        return false;
    }

    // Check if distancetOption is 'country' or 'state' and normalizedTitleOption contains 'Any'
    if ((this.distancetOption === 'country' || this.distancetOption === 'state') && this.normalizedTitleOption.includes('Any')) {
        alert('When Distance is "country" or "state",  Title must be selected.');
        return false;
    }

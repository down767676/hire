...
export class DefaultsTabComponent extends BaseTabComponent {
  public onClickRefreshWaitCursor: boolean = false;
  ...
  onClickRefresh() {
    this.onClickRefreshWaitCursor = this.showWait(this.onClickRefreshWaitCursor);
    this.dataService.fetchData(this.api_end_point, this.sp, {}).subscribe(data => {
      this.showGrid(data);
      this.onClickRefreshWaitCursor = this.hideWait(this.onClickRefreshWaitCursor);
    });
  }
